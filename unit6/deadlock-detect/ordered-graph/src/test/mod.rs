...
fn cond_k() {
    ...
    unsafe {
        ...
        Runtime::request_report(&mut rt as *mut Runtime, a, r).unwrap();
        Runtime::request_report(&mut rt as *mut Runtime, c, t).unwrap();
        Runtime::request_report(&mut rt as *mut Runtime, a, s).unwrap();
        assert_eq!(Runtime::request_report(&mut rt as *mut Runtime, c, r).unwrap_err(), ReqErrCode::ResOccupied);
        Runtime::release_report(&mut rt as *mut Runtime, a, r).unwrap();
        Runtime::release_report(&mut rt as *mut Runtime, a, s).unwrap();
        Runtime::release_report(&mut rt as *mut Runtime, c, t).unwrap();
        Runtime::release_report(&mut rt as *mut Runtime, c, r).unwrap();
    }
}